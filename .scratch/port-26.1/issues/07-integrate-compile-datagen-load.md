...
- **Manual testing aids:**
  ...
  - **Test datapack:** `.scratch/port-26.1/dev-datapacks/researchd-lab-test` adds a root research, `researchd:lab_test`, that consumes 3 Overworld Research Packs at 20 ticks each. It's copied into `run/saves/*/datapacks` and `run/port26-smoke/datapacks`. The research method id is `researchd:consume_research_pack`. Pack ids are `researchd:overworld`, `researchd:nether` and `researchd:end`; `researchd:overworld_pack` is a research.
